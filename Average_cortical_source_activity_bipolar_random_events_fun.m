...
%% This Script should average the cortical activity time-locked to random events
%This script is used to average cortical activity around random time points. 
...
subject_ID = {'S001','S003','S004', ...};switch Med_state
switch Med_state
    case 'ON'
        % ON
        subject = {...
    ...
    for iEEG = 1 : 2%length(EEG)
        for iSet = 1 : 100
            for iSubj= iSubject%loop over subjects
                ...
                if exist([database subject{iSubj,1} '/@intra/results_PNAI_avg_'  Detection_method '_' average_period '_random_events_base_set_' num2str(iSet) '_' subjects{iSubj} '_' Med_state '.mat'], 'file') == 2
                     continue
                end
                ...
                for irun=1:length(subject{iSubj,2}) % loop over runs 
                    ...
                    peak_Burst = Time(randperm(size(Time,2),Number_of_events));
                    start_Burst = peak_Burst-(randi(400,1,Number_of_events)+100)/1000;%alter the baseline offset betweem 100 and 500 ms
                    end_Burst = peak_Burst;
                    Ripple_tmp = ones(size(peak_Burst));                
                ...
                if ~isempty(sSource)
                    ...
                    for irun=1:length(Navg_N)
                        ...
                        if irun == 1
                            ...
                            delete([sSource{irun}])
                            av_source_filename_fin = [path_brainsotrm_data subject{iSubj,1} '/@intra/results_PNAI_avg_' Detection_method '_' average_period '_random_events_base_set_' num2str(iSet) '_' subject_ID{iSubj} '_Bipolar_' Hemispheres{iEEG} '_' Med_state '.mat'];                            
                    ...
                    Output_path = ['Outputpath/Cortical_random_set/' num2str(iSet) '/' subjects{iSubj} '/'];
                    if ~exist(Output_path, 'dir')
                        mkdir(Output_path)
                    end
                    copyfile([database subject{iSubj,1} '/@intra/results_PNAI_avg_'  Detection_method '_' average_period '_random_events_base_set_' num2str(iSet) '_' subjects{iSubj} '_' Med_state '.mat'],Output_path)
                    save([Output_path 'Random_events_timepoints_Set_' num2str(iSet) '_' subjects{iSubj} '.mat'],'Events_timepoints')
